...
export const Grocery: React.FC = () => {
  ...
  const handleToggleSelectAllLists = () => {
    if (selectedListIds.length === lists.length && lists.length > 0) {
      setSelectedListIds([]);
    } else {
      setSelectedListIds(lists.map((l) => l._id));
    }
  };
  ...
  const loadLists = async (preferredId?: string) => {
    ...
    try {
      ...
      // Default select all lists if there are 2 or more, enabling 1-click sharing of both lists
      if (fetchedLists.length > 1) {
        setSelectedListIds((prev) => (prev.length === 0 ? fetchedLists.map((l) => l._id) : prev));
      }
    } catch (err) {
  ...
  // Helper to determine which lists to share or copy
  const getListsToShare = (customLists?: GroceryList[]): GroceryList[] => {
    if (customLists && customLists.length > 0) {
      return customLists;
    }
    if (viewMode === "recipe" && activeRecipe) {
      return [
        {
          _id: "recipe",
          title: activeRecipe.title,
          items: recipeShoppingItems,
        },
      ];
    }
    if (selectedListIds.length > 1) {
      return lists.filter((l) => selectedListIds.includes(l._id));
    }
    if (selectedListIds.length === 1) {
      const found = lists.find((l) => l._id === selectedListIds[0]);
      if (found) return [found];
    }
    return selectedList ? [selectedList] : [];
  };

  const formatMultiListWhatsAppMessage = (listsToShare: GroceryList[]): string => {
    if (listsToShare.length === 1) {
      const l = listsToShare[0];
      let message = `🛒 *NutriVerse Shopping List*\n`;
      message += `List: *${l.title}*\n\n`;
      message += `*REQUIRED ITEMS TO BUY (${l.items.length}):*\n`;
    ...
    const totalItems = listsToShare.reduce((acc, l) => acc + l.items.length, 0);
    let message = `🛒 *NutriVerse Combined Shopping Lists* (${listsToShare.length} Dishes / Lists — ${totalItems} Items)\n\n`;

    listsToShare.forEach((l, idx) => {
      message += `📋 *${idx + 1}. ${l.title}* (${l.items.length} items):\n`;
      if (l.items.length === 0) {
  ...
  const formatMultiListClipboardText = (listsToShare: GroceryList[]): string => {
    if (listsToShare.length === 1) {
      const l = listsToShare[0];
      let text = `🛒 NutriVerse Shopping List — ${l.title}\n`;
      text += `Required Items (${l.items.length}):\n\n`;
      l.items.forEach((item) => {
        const check = item.purchased ? "✅" : "☐";
    ...
    const totalItems = listsToShare.reduce((acc, l) => acc + l.items.length, 0);
    let text = `🛒 NutriVerse Shopping Lists (${listsToShare.length} Lists — ${totalItems} Items)\n\n`;

    listsToShare.forEach((l, idx) => {
      text += `📋 ${idx + 1}. ${l.title} (${l.items.length} items):\n`;
      if (l.items.length === 0) {
  ...
  // Copy list(s) to clipboard
  const handleCopyList = async (customLists?: GroceryList[]) => {
    const targetLists = getListsToShare(customLists);

    if (targetLists.length === 0 || targetLists.every((l) => l.items.length === 0)) {
      showToast.error("No items to copy.");
      return;
    }

    const text = formatMultiListClipboardText(targetLists);
    ...
    try {
      ...
      showToast.success(
        targetLists.length > 1
          ? `Copied ${targetLists.length} lists to clipboard!`
  ...
  // Share list(s) to WhatsApp
  const handleShareWhatsApp = (customLists?: GroceryList[]) => {
    const targetLists = getListsToShare(customLists);
    ...
    const message = formatMultiListWhatsAppMessage(targetLists);
    const whatsappUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, "_blank");
  };

  // Combine multiple selected lists into a single consolidated list
  const handleMergeSelectedLists = async () => {
    const targetLists = selectedListIds.length > 1
      ? lists.filter((l) => selectedListIds.includes(l._id))
      : lists;

    if (targetLists.length < 2) {
      showToast.error("Please select at least 2 lists to combine.");
      return;
    }
    ...
    try {
      ...
      const cleanTitles = targetLists.map((l) => l.title);
      const title = `Combined: ${cleanTitles.slice(0, 2).join(" + ")}${cleanTitles.length > 2 ? ` (+${cleanTitles.length - 2})` : ""}`;
      ...
      setLists((prev) => [mergedList, ...prev]);
      setSelectedListId(mergedList._id);
      setSelectedListIds([mergedList._id]);
      showToast.success(`Combined ${targetLists.length} lists into "${title}"!`);
      navigate(`/grocery?listId=${mergedList._id}`, { replace: true });
    } catch (err) {
      console.error("Failed to combine lists:", err);
      showToast.error("Failed to combine grocery lists.");
    } finally {
      setIsMerging(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          {viewMode === "lists" && selectedListIds.length > 1 && (
            <button
              onClick={handleMergeSelectedLists}
              disabled={isMerging}
              className="flex items-center gap-1.5 px-4 py-2.5 bg-zinc-900 hover:bg-black dark:bg-zinc-800 dark:hover:bg-zinc-700 text-white rounded-xl text-sm font-bold shadow-sm transition cursor-pointer disabled:opacity-50"
              title="Combine selected lists into a single consolidated grocery list"
            >
              <Merge className="h-4 w-4" />
              <span>{isMerging ? "Merging..." : "Combine into 1 List"}</span>
            </button>
          )}
              ...
              <>
                ...
                <span>
                  {selectedListIds.length === 2
                    ...
                    ? `Copy Selected (${selectedListIds.length})`
                    : "Copy List"}
                </span>
              </>
            )}
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-xl text-sm font-bold shadow-sm transition cursor-pointer"
          ...
          >
            ...
            <span>
              {selectedListIds.length === 2
                ...
                : selectedListIds.length > 1
                ? `Share Selected (${selectedListIds.length}) on WhatsApp`
                : "Share on WhatsApp"}
            </span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-5 shadow-sm">
              ...
              {lists.length > 1 && (
                <div className="flex items-center justify-between bg-zinc-50 dark:bg-zinc-800/60 px-3 py-2 rounded-xl mb-3 border border-zinc-100 dark:border-zinc-800 text-xs">
                  <button
                    type="button"
                    onClick={handleToggleSelectAllLists}
                    className="flex items-center gap-1.5 font-bold text-zinc-700 dark:text-zinc-300 hover:text-amber-600 dark:hover:text-amber-400 transition cursor-pointer"
                  >
                    {selectedListIds.length === lists.length ? (
                      ...
                      <>
                        ...
                        <span>{lists.length === 2 ? "Select Both Lists" : `Select All (${lists.length})`}</span>
                      </>
                    )}
                  </button>
                  <span className="font-semibold text-zinc-500 dark:text-zinc-400">
                    {selectedListIds.length} of {lists.length} selected
                  </span>
                </div>
              )}
                          ...
                          <div className="flex items-start gap-2.5 min-w-0 flex-1">
                            {lists.length > 1 && (
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  toggleListSelection(list._id);
                                }}
                                className="mt-0.5 text-zinc-400 hover:text-amber-500 transition shrink-0 cursor-pointer"
                                title={isChecked ? "Deselect from multi-share" : "Select to share together"}
                              >
                                {isChecked ? (
                                  <CheckSquare className="h-4 w-4 text-amber-500" />
                                ) : (
                                  <Square className="h-4 w-4 text-zinc-300 dark:text-zinc-600 hover:text-amber-500" />
                                )}
                              </button>
                            )}
